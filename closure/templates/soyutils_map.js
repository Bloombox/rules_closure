...
function $$mapToLegacyObjectMap(map) {
  ...
  for (const k of map.keys()) {
    assertString(k);
    obj[/** @type {!string} */ (k)] = /** @type {!V} */ (map.get(k));
  }
  return obj;
}
...
function $$populateMap(jspbMap, map) {
  for (const k of map.keys()) {
    jspbMap.set(k, map.get(k));
  }
}

/**
 * Maybe coerce a key to a string.
 *
 * @param {!goog.soy.data.UnsanitizedText|string} value Unsanitized text or raw
 *        string content.
 * @return {!string} String content passed-in, or coerced.
 */
function $$maybeCoerceKeyToString(value) {
